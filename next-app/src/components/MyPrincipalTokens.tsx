...
import { principalListings } from "@/app/principal-market/principalListings";
import { MATURITY_6M, MATURITY_12M } from "@/constants";
import { useUserPTByMaturity } from "@/hooks/useTokenMinting";
import { SwapHoriz } from "@mui/icons-material";

interface UserPrincipalToken {
  protocol: string;
  title: string;
  maturity: string;
...
export default function MyPrincipalTokens() {
  const { address } = useAccount();
  const [userTokens, setUserTokens] = useState<UserPrincipalToken[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [totalPTBalance, setTotalPTBalance] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const tokens: UserPrincipalToken[] = [];
    let total = 0;
    ...
    balanceQueries.forEach(({ protocol, maturity, maturityIndex, data }) => {
      if (data.data !== undefined && data.data !== null) {
        ...
        if (balance > 0) {
          ...
          const listing = principalListings.find(
  ...
  const handleTradeToken = (protocol: string, maturity: string) => {
    ...
    window.location.href = `/principal-market/${protocolName}${maturitySuffix}`;
  };
          ...
          <Card sx={commonStyles.cardStyles}>
            <CardContent sx={{ p: 3, textAlign: "center" }}>
              <Typography variant="h6" color="text.secondary" sx={{ mb: 2 }}>
                Please connect your wallet to view your principal tokens
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Connect your wallet to see your principal token holdings and
                total value.
              </Typography>
            </CardContent>
          </Card>
          ...
          <CardContent sx={{ p: 4 }}>
            <Stack spacing={2}>
              ...
              {isLoading ? (
                ...
                >
                  {totalPTBalance.toFixed(2)} PT
                </Typography>
              )}
              <Typography variant="body2" color="text.secondary">
                {totalPTBalance > 0
                  ? `Your principal tokens represent up to $${totalPTBalance.toFixed(
                      2,
                    )} of redeemable value at maturity.`
                  : "You don't hold any principal tokens yet."}
              </Typography>
            </Stack>
          </CardContent>
        </Card>

        {/* Principal Tokens Grid */}
        <Box sx={{ mb: 4 }}>
          {isLoading ? (
            <Stack spacing={3}>
              {[1, 2, 3].map((i) => (
              ...
              {userTokens.map((token, index) => (
                ...
                >
                  ...
                  <Button
                    ...
                    startIcon={<SwapHoriz />}
                    onClick={() =>
                      handleTradeToken(token.protocol, token.maturity)
                    }
                    sx={{
                      borderColor: "#3b82f6",
                      color: "#3b82f6",
            ...
            <Card sx={commonStyles.cardStyles}>
              <CardContent sx={{ p: 4, textAlign: "center" }}>
                ...
                <Typography
                  ...
                  sx={{ mb: 3 }}
                >
                  You don&apos;t currently hold any principal tokens. Mint
                  tokens to receive principal tokens.
                </Typography>
                <Button
                  variant="contained"
