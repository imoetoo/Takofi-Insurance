...
import { parseInsuranceMetrics } from "@/utils/insuranceCalculations";
import {
...
const getPrincipalData = (protocolName: string): PrincipalData | undefined => {
  const principalMap: { [key: string]: PrincipalData } = {
    sushiswap: {
      title: "SushiSwap",
      provider: "SushiSwap Principal Token",
      protocol: "exchange",
      isNew: true,
    },
    "curve-finance": {
      title: "Curve Finance",
      provider: "Curve Finance Principal Token",
      protocol: "defi",
      isNew: false,
    },
    aave: {
      title: "Aave",
      provider: "Aave Principal Token",
      protocol: "lending",
      isNew: false,
    },
    "uniswap-v3": {
      title: "Uniswap V3",
      provider: "Uniswap Principal Token",
      protocol: "exchange",
      isNew: false,
    },
    compound: {
      title: "Compound",
      provider: "Compound Principal Token",
      protocol: "lending",
      isNew: false,
    },
    pancakeswap: {
      title: "PancakeSwap",
      provider: "PancakeSwap Principal Token",
      protocol: "exchange",
      isNew: false,
    },
  };

  return principalMap[protocolName];
};
...
export default function PrincipalDetailPage() {
  ...
  // Get token addresses - use principalToken instead of insuranceToken
  ...
  // Use DEX hook with maturity-specific order book for PT
  const {
    orderBook,
    userOrders,
    bestBuyPrice,
  ...
  // Use maturity-specific balance hook for PT
  const { data: principalBalance } = useUserPTByMaturity(
    address,
    principalData?.title || protocolName,
    maturityIndex,
  );
  ...
  // Get principal token allowance and approval for DEX
  const { allowance: principalAllowance, approve: approvePrincipal } = useERC20(
    principalTokenAddress || ("0x0" as `0x${string}`),
    address,
    DEX_CONTRACT_ADDRESS,
  );
  ...
  if (!principalData || !principalTokenAddress) {
    return (
      <Box sx={commonStyles.pageContainerStyles}>
        <Container maxWidth="lg">
          <Typography variant="h4" color="text.primary">
            Principal listing not found
          </Typography>
        </Container>
      </Box>
    );
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const amountBigInt = parseUnits(amount, 18); // PT amount (18 decimals)
      const priceBigInt = parseUnits(price, 18); // Price scaled by 1e18

      // Calculate required quote tokens (USDT/USDC with 6 decimals)
      // Formula: (PT_amount * price) / PRICE_PRECISION / 1e12 = quote_amount in 6 decimals
      ...
      if (tradeType === "sell") {
        // Selling PT: need PT approval
        if (principalAllowance < amountBigInt) {
          setSuccess("Approving Principal Token...");
          await approvePrincipal(amountBigInt);
        }
      } else {
        // Buying PT: need stablecoin approval
  ...
  const handleTakeOrder = async (order: Order) => {
    ...
    try {
      ...
      if (order.action === 1) {
        ...
        // Order is BUY, so we need to SELL (pay with PT)
        if (principalAllowance < amountToTake) {
          setSuccess("Approving Principal Token...");
          await approvePrincipal(amountToTake);
        }
      }
  ...
  return (
    <Box sx={commonStyles.pageContainerStyles}>
      <Container maxWidth="xl">
        ...
        {/* Header with Principal Token Info */}
          ...
          <CardContent sx={{ p: 3 }}>
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
                ...
                >
                  <Image
                    src={getProtocolLogo(principalData.title)}
                    alt={`${principalData.title} logo`}
                    width={48}
                    height={48}
                  ...
                  >
                    ...
                    <Box sx={{ mt: 2 }}>
                      <Typography
                        variant="h5"
                        sx={{
                          color: getProtocolColor(principalData.protocol),
                          fontWeight: "bold",
                        }}
                      >
                        TVL:{" "}
                      ...
                      >
                        Total Principal Tokens in market
                      </Typography>
                    </Box>
                  </Box>
                </Box>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <Stack spacing={3}>
                <Box>
                  ...
                  {isConnected && (
                    <Typography variant="caption" color="text.secondary">
                      ...
                      {tradeType === "sell"
                        ? parseFloat(
                            formatUnits(principalBalance || BigInt(0), 18),
                          ).toFixed(4)
                        : "∞"}{" "}
                      PT
                    </Typography>
                ...
                >
                  ...
                  >
                    <Typography variant="body2" color="text.secondary">
                      {tradeType === "buy" ? stablecoin : "PT"} Balance:
                    </Typography>
                    <Typography variant="body2" color="text.primary">
                      {isConnected
                        ? tradeType === "buy"
                ...
                <Box sx={{ textAlign: "center" }}>
                  <Typography variant="caption" color="text.secondary">
                    Trading {principalData.title} PT with {stablecoin}
                  </Typography>
                </Box>
              </Stack>
            </CardContent>
