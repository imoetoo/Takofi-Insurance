...
async function main() {
  try {
    ...
    const protocolInsuranceAddress = "0xDC11f7E700A4c898AE5CAddB1082cFfa76512aDD";
    const protocolInsurance = await ethers.getContractAt("ProtocolInsurance", protocolInsuranceAddress);
    
    console.log(`👤 User address: ${userAddress}`);
    console.log(`📍 Contract address: ${protocolInsuranceAddress}`);
    
    // Check all protocols (matching smart contract constructor)
    const protocols = ["SushiSwap", "Curve Finance", "Aave", "Uniswap V3", "Compound", "MakerDAO"];
    ...
    for (const protocolName of protocols) {
      try {
        ...
        const insuranceToken = await ethers.getContractAt("InsuranceToken", protocolInfo.insuranceToken);
        const principalToken = await ethers.getContractAt("PrincipalToken", protocolInfo.principalToken);
        ...
        console.log(`${insuranceSymbol}: ${ethers.formatUnits(insuranceBalance, 18)}`);
        console.log(`${principalSymbol}: ${ethers.formatUnits(principalBalance, 18)}`);
