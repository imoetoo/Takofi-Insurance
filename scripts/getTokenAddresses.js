  ...
  try {
    ...
    for (const protocolName of protocols) {
      try {
        ...
        const insuranceToken = await ethers.getContractAt("InsuranceToken", insuranceTokenAddress);
        const principalToken = await ethers.getContractAt("PrincipalToken", principalTokenAddress);
        ...
        console.log(`❌ Error getting ${protocolName} addresses: ${error.message}`);
      }
    }
