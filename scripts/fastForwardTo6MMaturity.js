...
async function main() {
  ...
  const tokenMinting = await hre.ethers.getContractAt("ProtocolInsurance", tokenMintingAddress);
  ...
  console.log(`6M Maturity expiry time: ${new Date(expiryTime * 1000).toLocaleString()}`);
  console.log(`Days until expiry: ${Math.ceil((expiryTime - currentTime) / 86400)}\n`);

  // Fast forward to 1 day after 6M maturity expiry
  const timeToSkip = expiryTime - currentTime + 86400; // +1 day past expiry

  console.log(`⏳ Advancing time by ${Math.ceil(timeToSkip / 86400)} days...\n`);
  ...
  if (!maturityAfter[3]) { // Check if not already settled
  ...
  console.log("🎉 Ready to redeem! Your 6M principal tokens are now eligible for redemption.\n");
